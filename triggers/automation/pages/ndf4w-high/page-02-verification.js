/**
 * Page 2: Verification (Customer, Asset, Loan Structure)
 * Form: form_verification_show
 */
export const Page2Verification = (taskId, params = {}) => ({
  formName: "form_verification_show",
  payload: {
    "$.documents.ktp.document_id": "9c93b777-1a00-4def-9643-60cff93b75e4",
    "$.documents.selfie.document_id": "7fe1b2ac-6a02-42d0-b57c-ceb9a2bd5aa3",
    "$.documents.alternate_identity_document.document_id": "",
    "$.customer.ktp.name": params.customerName,
    "$.customer.ktp.birth_place": "MANOKWARI",
    "ltw.custom_marital_status": "Single",
    "ltw.reassignee_branch_id": "",
    "ltw.reassignee_pos_id": "",
    "ltw.reassignee_branch_name": "",
    "ltw.reassignee_name": "",
    "ltw.new_appointment_id": "",
    "ltw.custom_vd_assignee": "",
    "$.customer.personal.number_dependents_children": "1",
    "$.customer.personal.number_dependents_other": "1",
    "$.documents.spouse_ktp.document_id": "",
    "$.spouse.ktp.name": "",
    "$.spouse.ktp.nik": "",
    "$.spouse.ktp.birth_place": "",
    "$.spouse.ktp.birth_date": "",
    "$.spouse.mobile_number": "",
    "ltw.custom_spouse_occupation_code": "",
    "$.customer.domicile.address.street_address": "JL KEBAGUSAN AJA",
    "$.customer.domicile.address.sub_district_code": "32.73.27.1003",
    "$.customer.domicile.address.rt": "001",
    "$.customer.domicile.address.rw": "001",
    "ltw.custom_formatted_address": "",
    "$.customer.domicile.address.location.latitude": "-6.9589899",
    "$.customer.domicile.address.location.longitude": "107.691285",
    "$.customer.domicile.stay_since": "2012",
    "ltw.custom_domicile_ownership_code": "Sendiri",
    "$.documents.stnk.document_id": "b91c0e6a-c0d7-4f20-938a-59af22ae8b97",
    "$.documents.tax_notice.document_id":
      "8715422b-ff16-4adb-b35b-88297eb4112b",
    "$.asset.license_plate": params.licensePlate,
    "ocr-suggestion--ltw.asset_license_plate": params.licensePlate,
    "ltw.asset_license_plate": params.licensePlate,
    "$.asset.asset_code": "DAIHATSU.AYLA.RDELUXE12AT",
    "ocr-suggestion--$.asset.manufacturing_year": "2019",
    "$.asset.manufacturing_year": "2019",
    "$.asset.tax_expiration_date": "2025-06-13",
    "$.asset.stnk_expiration_date": "2029-06-13",
    "$.process.stnk_ocr_result.unit_color": "HITAM METALIK",
    "$.asset.chassis_number": "MJLV8DA2H7K001231",
    "$.process.stnk_ocr_result.chassis_number": "MJLV8DA2H7K001231",
    "$.process.stnk_ocr_result.engine_number": "2NDS903234",
    "$.asset.stnk_number": "",
    "$.asset.bpkb_status": "on_hand",
    "$.asset.bpkb_ownership": "1",
    "ltw.custom_vehicle_guaranteed": "OWNED_BY_DEBTOR",
    "ltw.custom_bpkb_ownership_period": "FROM_6_TO_12_MONTH",
    "$.process.stnk_ocr_result.stnk_number": "",
    "$.process.stnk_ocr_result.license_plate": params.licensePlate,
    "$.process.stnk_ocr_result.vehicle_brand": "HONDA",
    "$.process.stnk_ocr_result.manufacturing_year": "2023",
    "$.process.stnk_ocr_result.stnk_expiration_date": "2029-06-13",
    "$.process.stnk_ocr_result.tax_expiration_date": "2025-06-13",
    "$.process.stnk_ocr_result.ocr_process_id":
      "8a75ed58-37de-416a-9969-b2d029b3a4d3",
    "$.process.stnk_ocr_result.vehicle_model": "SCOOTER",
    "$.process.stnk_ocr_result.registration_year": "2023",
    "$.loan_structure.provisional_amount": "72400000",
    "$.loan_structure.tenure": "12",
    "$.loan_structure.max_funding": "72400000",
    "$.loan_structure.monthly_installment": "7027500",
    "$.loan_structure.interest_rate": "0.165",
    "$.loan_structure.life_insurance_premium": "425000",
    "$.loan_structure.admin_fee": "2200000",
    "$.loan_structure.provision_fee": "905000",
    "$.loan_structure.capitalized_ntf_amount": "77249500",
    "$.loan_structure.ltv": "0.749995",
    "$.loan_structure.funding_ratio": "0.702913",
    "$.process.digital_ltv_max_ndf4w.s1.ltv_max_ratio": "0.75",
    action: "submit",

    // for single
    "ltw.custom_marital_status": "Single",
    "$.spouse.ktp.name": "",
    "$.spouse.ktp.nik": "",
    "$.spouse.ktp.birth_place": "",
    "$.spouse.ktp.birth_date": "",
    "$.spouse.mobile_number": "",
    "$.documents.spouse_ktp.document_id": "",
    "ltw.custom_spouse_occupation_code": "",

    // for married
    // "ltw.custom_marital_status": "Married",
    // "$.spouse.ktp.name": "Sinta",
    // "$.spouse.ktp.nik": "3174092409580043",
    // "$.spouse.ktp.birth_place": "Jakarta",
    // "$.spouse.ktp.birth_date": "2009-12-02",
    // "$.spouse.mobile_number": "+628131743692",
    // "$.documents.spouse_ktp.document_id":
    //   "58244ce3-f49b-43d7-9198-b559fc7bfff3",
    // "ltw.custom_spouse_occupation_code": "Accounting/Finance Officer",
  },
});
