/**
 * Page 8: Asset & Document Mandatory (LOW RISK)
 * Form: form_asset_document_mandatory_show
 * Comprehensive asset photos, BPKB pages, and all customer documents
 */
export const Page8AssetDocumentMandatoryLow = (taskId, params = {}) => ({
  formName: "form_asset_document_mandatory_show",
  payload: {
    "$.documents.asset.asset_front.document_id":
      "a27afbf6-b892-4771-a5c0-d861908da312",
    "$.documents.asset.asset_rear.document_id":
      "7e8e7f5e-b26a-438e-927f-d46ebb6566aa",
    "$.documents.asset.asset_left_side.document_id":
      "7a1e08d1-c160-4f51-a251-7e3995a47551",
    "$.documents.asset.asset_right_side.document_id":
      "7f071dc1-293f-4296-9275-f3275237e578",
    "$.documents.asset.asset_front_left_side.document_id":
      "892b00fb-f949-406d-99a6-e15f62e5d28a",
    "$.documents.asset.asset_front_right_side.document_id":
      "7b41e2dd-f77b-494b-bf61-9480ada1a43b",
    "$.documents.asset.asset_rear_left_side.document_id":
      "578e0983-88ff-46be-9bf0-8ace0305de77",
    "$.documents.asset.asset_rear_right_side.document_id":
      "b7e0e28f-d60e-4ab6-9660-4a65a7262135",
    "$.documents.asset.asset_interior_front_side.document_id":
      "ee64aa01-e80f-4c12-adb5-c3f70c45a4b2",
    "$.documents.asset.asset_interior_rear_side.document_id":
      "159d7d4f-a610-4e38-be62-435a04978983",
    "$.documents.asset.asset_interior_center_sid.document_id":
      "ed978e50-769b-4276-8f69-e9d5fe6b8316",
    "$.documents.asset.speedometer.document_id":
      "a94df889-8228-44ce-bbf3-525d7652eff5",
    "$.documents.asset.engine_number.document_id":
      "4ae38ccc-8aac-484d-a622-da3159b3a348",
    "$.documents.asset.selfie_with_vehicle.document_id":
      "9e70af43-ce7f-4a7f-a004-ad0bef8b3cc4",
    "$.documents.bpkb_page_1.document_id":
      "886966d8-3e47-4b85-a196-1ac4c84eb950",
    "$.documents.bpkb_page_2.document_id":
      "bd6f08df-30d2-441d-8042-dbf7a54e742c",
    "$.documents.bpkb_page_3.document_id":
      "4a8c9d10-5cd6-4472-8f75-c1f21e6a58aa",
    "$.documents.bpkb_page_4.document_id":
      "bea78741-167d-4390-8eff-6e46a20b0460",
    "$.documents.bpkb_page_5.document_id": "",
    "$.documents.stnk.document_id": "173c8fa6-89ed-4f65-b807-f9281528b92e",
    "$.documents.tax_notice.document_id": "3ea93a56-df3f-445a-b6f3-b4741e442e74",
    "$.documents.chassis_number.document_id":
      "3c89f85d-4a02-4c8b-accf-a52bb2df693a",
    "$.documents.bpkb_invoice.document_id": "",
    "$.documents.vehicle_inspection.document_id": "",
    "$.documents.release_letter.document_id": "",
    "ltw.is_bpkb_duplicate": "false",
    "ocr-suggestion--$.asset.unit_color": "VIOLET SILVER",
    "$.asset.unit_color": "VIOLET SILVER",
    "ocr-suggestion--$.asset.chassis_number": "PASSDHEHIU2898929",
    "$.asset.chassis_number": "PASSDHEHIU2898929",
    "ocr-suggestion--$.asset.engine_number": "JF71E1159387",
    "$.asset.engine_number": "JF71E1159387",
    "$.asset.bpkb_number": "TESTSETSET",
    "$.asset.bpkb_invoice_number": "TESTSETTEST",
    "$.asset.bpkb_ownership_proof": "1",
    "$.asset.bpkb_owner_name": "NEW DATA",
    "$.process.bpkb_ocr_result.vehicle_brand": "HONDA",
    "$.process.bpkb_ocr_result.chassis_number": "MH1JF7111BK160084",
    "$.process.bpkb_ocr_result.engine_number": "JF71E1159387",
    "$.process.bpkb_ocr_result.manufacturing_year": "2011",
    "$.process.bpkb_ocr_result.license_plate": "AD2306UY",
    "$.process.bpkb_ocr_result.bpkb_owner_nik": "",
    "$.process.bpkb_ocr_result.bpkb_owner_name": "",
    "$.process.bpkb_ocr_result.bpkb_address": "",
    "$.process.bpkb_ocr_result.bpkb_number": "",
    "$.process.bpkb_ocr_result.bpkb_invoice_number": "",
    "$.process.bpkb_ocr_result.unit_color": "VIOLET SILVER",
    "$.asset.bpkb_address": "",
    "$.documents.ktp.document_id": "1a298e4a-7f28-446d-bb0a-fa47db34c173",
    "$.documents.selfie.document_id": "a7e788ad-96df-4796-bafb-01bc383766b3",
    "$.documents.employment_evidence.document_id":
      "78190841-eb00-472a-8a48-86dd337f2757",
    "$.documents.npwp.document_id": "c61874ba-4b9b-4b0b-8320-d4df0122fd22",
    "$.documents.income_proof.document_id":
      "796746a1-c893-4421-9cff-bc3e487eb786",
    "$.documents.business_legality.document_id": "",
    "$.documents.kk.document_id": "3d7f52f4-cd27-42b0-89b4-3ff9282c5981",
    "$.documents.alternate_identity_document.document_id": "",
    "$.documents.marriage_certificate.document_id": "",
    "$.documents.divorce_certificate.document_id": "",
    "$.documents.death_certificate.document_id": "",
    "$.documents.house_ownership.document_id":
      "cbbbc7d1-f10e-4454-bc1c-1ab9b0ec1e92",
    "$.documents.debtor_signature.document_id":
      "c50fd053-f69b-4cbc-b829-f6b0cd753458",
    "$.documents.bpkb_receipt_2.document_id":
      "06050f6e-92b7-4ec8-9f49-2e9f8890201b",
    "$.documents.customer_receipt_2.document_id":
      "7689dfb3-bf02-4f71-94cc-8a7cb67c56e0",
    "$.documents.payment_receipt.document_id":
      "aca973de-a6ae-46ae-9735-b3a8671e9294",
    action: "submit",
  },
});
