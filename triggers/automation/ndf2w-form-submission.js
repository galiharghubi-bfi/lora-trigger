...
import { NDF2W_FORM_SEQUENCE } from "./pages/index.js";
...
export const waitForTask = async (
  ...
  console.log(
    `Polling for task creation (retry every ${
      retryInterval / 1000
  ...
  while (Date.now() - startTime < timeout) {
    ...
    if (taskId) {
      console.log(
        `✔ Found task ID: ${taskId} (after ${attempt} attempt${
          attempt > 1 ? "s" : ""
    ...
    try {
      ...
      if (currentFormName !== expectedFormName) {
        ...
        if (actualIndex > i) {
          ...
          console.log(
            `↻ Workflow is ahead! Syncing from page ${pageNumber} to page ${
              actualIndex + 1
            }`
          );
          ...
          console.log(
            `⚠ Workflow is behind (expected page ${pageNumber}, but on page ${
              actualIndex + 1
            }). Submitting expected form anyway.`
          );
 ...
 * @returns {Promise<void>}
 */
export const Ndf2wFormAutomation = async (
  workflowId,
  userId,
  licensePlate,
  customerName,
  ...
  try {
    console.log(`\n${"=".repeat(60)}`);
    console.log(`Starting 2W ${riskLevel.toUpperCase()} Risk Form Automation`);
    ...
    const sequence = customSequence || NDF2W_FORM_SEQUENCE;
    ...
    if (!isCompletionPage) {
      ...
      console.log(
        `\n✔ Final task state: ${
          finalTaskDetail.form_name || finalTaskDetail.formName
        }`
      );
    } else {
      console.log(
